...
for (classVar in classVarRange) {
  for (noiseVar in noiseVarRange) {
    probThreeData <- generateData(nObs = 500, nClassVars = classVar, 
                                  nNoiseVars = noiseVar, deltaClass = 0.5)
...
``` {r plotProbThree}
ggplot(probThreeResults, aes(x = fct_reorder(classifier, acc), y = acc, col = classVars)) + 
  geom_point() + facet_grid(noiseVars ~ ., scales = "free_y")
```
